...
6. **Webhook adapter** receives signed attestation and calls the USD Gateway (your Stripe backend) with `order`, `amountUSD`, `recipientEmail` or `merchantId`.
7. **Stripe** processes the charge or creates a gift card code; returns success + reference id.
8. Adapter writes settlement back to the ledger (off-chain DB) and optionally calls an on-chain callback `attestor.settleTx()` if required by protocol.
9. Frontend displays success; user receives gift card or money movement confirmation.
...
4. **Frontend mispricing** — using naive ratio instead of TWAP.

   * *Mitigation*: Always call `quoteSqrtPriceX96` and convert with local verified helper; show slippage & TWAP.
5. **Adapter API abuse** — replay or unauthorized calls to merchant APIs.
